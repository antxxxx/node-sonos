 ...
 * @param  {String}   searchType  Choice - artists, albumArtists, albums, genres, composers, tracks, playlists, share
 * @param  {String}   searchTerm  search term to search for
 * @param  {Object}   options     default {start: 0, total: 100}, apiKey for apigee, username for spotify, password for spotify, apigeeurl
 * @param  {Function} callback (err, result) result - {returned: {String}, total: {String}, items:[{title:{String}, uri: {String}}]}
 */
Sonos.prototype.searchSpotify = function (searchType, searchTerm, options, callback) {
    var self = this
    client = new Client();
    var searches = {
        'artists': 'A:ARTIST',
	...
	if (options.start !== undefined) StartingIndex = options.start
	if (options.total !== undefined) RequestedCount = options.total
  
    sessionidargs = {
        parameters: { apikey: options.apikey, username: options.username, password: options.password }
    };
    client.get(options.apigeeurl + "/sessionid", sessionidargs, function (sessioniddata, sessionidresponse) {
        if (sessionidresponse.statusCode == 200) {
            args = {
                parameters: { apikey: options.apikey, id: "strk", term: searchTerm, index: StartingIndex, count: RequestedCount, sessionid: sessioniddata.getSessionIdResponse.getSessionIdResult }
            };
            client.get(options.apigeeurl + "/search", args, function (data, response) {
                if (response.statusCode == 200) {
                    var items = []
					if (data.searchResponse.searchResult.count > 0) {
						data.searchResponse.searchResult.mediaMetadata.forEach(function (item) {
							var id = item.id
							var title = item.title
							var trackMetadata = item.trackMetadata
							var artist = trackMetadata.artist
							var album = trackMetadata.album
							var albumId = trackMetadata.albumId
							var albumArtURL = trackMetadata.albumArtURI
...
'</DIDL-Lite>'
                    ...
                    var result = {
                        returned: data.searchResponse.searchResult.count,
                        total: data.searchResponse.searchResult.total,
                        items: items
                    }
                    return callback(null, result)
                }
